...
func (dao *UserDao) SearchUsers(condition *UserSearchCondition, rowbound *model.RowBound) (result []do.User, count int, err error) {
	db := dao.db
	...
	if rowbound == nil {
		err = db.Model(&do.User{}).Order("ID desc").Count(&count).Find(&result).Error
	} else {
		err = db.Model(&do.User{}).Order("ID desc").Count(&count).Offset(rowbound.Offset).Limit(rowbound.Limit).Find(&result).Error
	}
