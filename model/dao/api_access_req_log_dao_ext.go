...
type APIAccessReqLogSearchCondition struct {
	//id
	IDS []int64
	//创建开始时间
...
func (dao *APIAccessReqLogDao) SearchAPIAccessReqLogs(condition *APIAccessReqLogSearchCondition, rowbound *model.RowBound) (result []do.APIAccessReqLog, count int, err error) {
	db := dao.db
	...
	if rowbound == nil {
		err = db.Model(&do.APIAccessReqLog{}).Order("ID desc").Count(&count).Find(&result).Error
	} else {
		err = db.Model(&do.APIAccessReqLog{}).Order("ID desc").Count(&count).Offset(rowbound.Offset).Limit(rowbound.Limit).Find(&result).Error
	}
