...
func (dao *OrderDao) SearchOrders(condition *OrderSearchCondition, rowbound *model.RowBound) (result []do.Order, count int, err error) {
	...
	if rowbound == nil {
		err = db.Model(&do.Order{}).Order("ID desc").Count(&count).Find(&result).Error
	} else {
		err = db.Model(&do.Order{}).Order("ID desc").Count(&count).Offset(rowbound.Offset).Limit(rowbound.Limit).Find(&result).Error
	}
