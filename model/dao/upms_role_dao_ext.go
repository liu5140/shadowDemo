...
type UpmsRoleSearchCondition struct {
    //id
    IDS []int64
	//创建开始时间
...
func (dao *UpmsRoleDao) SearchUpmsRoles(condition *UpmsRoleSearchCondition, rowbound *model.RowBound) (result []do.UpmsRole, count int, err error) {
	db := dao.db
	...
	if rowbound == nil {
		err = db.Model(&do.UpmsRole{}).Order("ID desc").Count(&count).Find(&result).Error
	} else {
		err = db.Model(&do.UpmsRole{}).Order("ID desc").Count(&count).Offset(rowbound.Offset).Limit(rowbound.Limit).Find(&result).Error
	}
