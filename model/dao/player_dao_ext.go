...
type PlayerSearchCondition struct {
    //id
    IDS []int64
	//创建开始时间
...
func (dao *PlayerDao) SearchPlayers(condition *PlayerSearchCondition, rowbound *model.RowBound) (result []do.Player, count int, err error) {
	db := dao.db
	...
	if rowbound == nil {
		err = db.Model(&do.Player{}).Order("ID desc").Count(&count).Find(&result).Error
	} else {
		err = db.Model(&do.Player{}).Order("ID desc").Count(&count).Offset(rowbound.Offset).Limit(rowbound.Limit).Find(&result).Error
	}
