...
type MerchantSearchCondition struct {
	//id
	IDS []int64
	//创建开始时间
...
func (dao *MerchantDao) SearchMerchants(condition *MerchantSearchCondition, rowbound *model.RowBound) (result []do.Merchant, count int, err error) {
	db := dao.db
	...
	if rowbound == nil {
		err = db.Model(&do.Merchant{}).Order("ID desc").Count(&count).Find(&result).Error
	} else {
		err = db.Model(&do.Merchant{}).Order("ID desc").Count(&count).Offset(rowbound.Offset).Limit(rowbound.Limit).Find(&result).Error
	}
