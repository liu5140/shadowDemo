...
type ProgConfigSearchCondition struct {
	//id
	IDS []int64
	//创建开始时间
...
func (dao *ProgConfigDao) SearchProgConfigs(condition *ProgConfigSearchCondition, rowbound *model.RowBound) (result []do.ProgConfig, count int, err error) {
	...
	if rowbound == nil {
		err = db.Model(&do.ProgConfig{}).Order("ID desc").Count(&count).Find(&result).Error
	} else {
		err = db.Model(&do.ProgConfig{}).Order("ID desc").Count(&count).Offset(rowbound.Offset).Limit(rowbound.Limit).Find(&result).Error
	}
